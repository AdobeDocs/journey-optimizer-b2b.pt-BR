...
Para adicionar conteúdo condicional às suas mensagens de email, o Adobe Journey Optimizer permite aplicar regras condicionais armazenadas na biblioteca _Condições_. Aplique regras condicionais no designer de email ao [criar um email em uma jornada de conta](./email-authoring.md).
...
Ao criar um email para sua jornada de conta no designer de email, use regras condicionais para definir várias variantes para um componente de conteúdo.
...
1. Passe o mouse sobre a primeira variante (_Variante - 1_) e clique no ícone _Selecionar condição_ ( ![Ícone de condição](../assets/do-not-localize/icon-select-condition.svg) ).
   ...
   Se a condição necessária não existir, [crie uma regra condicional](#create-a-conditional-rule) clicando em **[!UICONTROL Criar novo]**.
...
As regras condicionais são um conjunto de expressões condicionais que podem ser avaliadas como true ou false. Você pode usar essas regras para determinar qual variante de conteúdo exibir em uma mensagem de email com base em vários critérios, como atributos de perfil ou eventos contextuais.
...
Você pode acessar o construtor de regras condicionais no designer de email ao selecionar uma condição para uma variante de componente.
